...
def gftin(cropped_las, r_max=2.0, alpha_max=0.2):
    ...
    while e_r < r_max and e_alpha < alpha_max and len(allpts)>0:
        ...
        for i in range(len(allpts)):
            pt = allpts[i]
            for i in range(len(new_dl.triangles)):
                tri = list(new_dl.triangles[i])
                print(1)
                ...
                if e_r > r and e_alpha > alpha:              # 写到一行对吗，不确定？？？？？？？？？？？
                    ...
                    e_alpha = alpha
                    i_selected = i
        new_dl.points.append(allpts[i_selected])
        new_dl.calculate_tri()
        del allpts[i_selected]
